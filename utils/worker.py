...
class Worker(object):
    ...
    def work(self,COORD):
        ...
        while not COORD.should_stop() and self.sess.run(self.global_step) < self.settings["MAX_EP"]:

            self.sess.run(self.global_step_next)

            logging = interval_flag(self.sess.run(self.global_step), self.settings["LOG_FREQ"], 'log')
            saving = interval_flag(self.sess.run(self.global_step), self.settings["SAVE_FREQ"], 'save')

            s0 = self.env.reset()

            for j in range(self.settings["MAX_EP_STEPS"]+1):
                updating = interval_flag(j, self.settings['UPDATE_GLOBAL_ITER'], 'update')
