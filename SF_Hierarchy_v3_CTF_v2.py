...
def ConstructSample(env,position):
    ...
    grid[position[0],position[1],4] = 5
    return grid
...
def UseSubpolicy(s,subpolicyNum):
    #Creating series of modified states that represent 1 step ahead.
    locx,locy = np.unravel_index(np.argmax(s[0,:,:,4], axis=None), s[0,:,:,4].shape)
    agent_id = s[0,locx,locy,4]
    s_blank = s
    s_blank[0,locx,locy,4] = 0

    s_up = s_blank.copy()
    if locy-1 < 0:
        s_up[0,locx,locy,4] = agent_id
    elif s_blank[0,locx,locy-1,3] == 1:
        s_up[0,locx,locy,4] = agent_id
    else:
        s_up[0,locx,locy-1,4] = agent_id

    s_down = s_blank.copy()
    if locy+1 > s.shape[1]-1:
        s_down[0,locx,locy,4] = agent_id
    elif s_blank[0,locx,locy+1,1] == 1:
        s_down[0,locx,locy,4] = agent_id
    else:
        s_down[0,locx,locy+1,4] = agent_id

    s_right = s_blank.copy()
    if locx+1 > s.shape[2]-1:
        s_right[0,locx,locy,4] = agent_id
    elif s_blank[0,locx+1,locy,1] == 1:
        s_right[0,locx,locy,4] = agent_id
    else:
        s_right[0,locx+1,locy,4] = agent_id

    s_left = s_blank.copy()
    if locy-1 < 0:
        s_left[0,locx,locy,4] = agent_id
    elif s_blank[0,locx,locy-1,1] == 1:
        s_left[0,locx,locy,4] = agent_id
    else:
        s_left[0,locx-1,locy,4] = agent_id
    #Feed through Phi estimate.

    phi = SF3.predict(np.vstack([s,s_up,s_right,s_down,s_left]))

    v = np.matmul(phi,v_select[subpolicyNum])
    #return argmax of the value
    return np.unravel_index(np.argmax(v, axis=None), v.shape)
