...
from methods.NGU import WorkerSlave,WorkerMaster,NGUBuffer
...
with tf.device(args.processor):
    ...
    for i in range(settings["NumberENV"]):
        ...
        Method = GetFunction(settings["Method"])
        localNetwork = Method(network,sess,stateShape=dFeatures,actionSize=nActions,scope=i_name,HPs=settings["NetworkHPs"],globalNet=Updater,nTrajs=nTrajs,sharedBuffer=sharedBuffer)
