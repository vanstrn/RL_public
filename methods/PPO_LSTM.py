...
class PPO(Method):
    ...
    def GetAction(self, state, global_step, step=1):
        ...
        try:
            probs,log_logits,v,hidden_state = self.sess.run([self.a_prob,self.log_logits,self.v,self.hidden_state], {self.s: state, self.hidden_state_prev:hidden_state_prev[0],self.hidden_cell_prev:hidden_state_prev[1]})

        except ValueError:
            probs,log_logits,v,hidden_state = self.sess.run([self.a_prob,self.log_logits,self.v,self.hidden_state], {self.s: np.expand_dims(state,axis=0), self.hidden_state_prev:hidden_state_prev[0],self.hidden_cell_prev:hidden_state_prev[1]})
        probs[:,2] *= 2.0
        probs[:,1] *= 0.75
        probs[:,0] *= 0.75
