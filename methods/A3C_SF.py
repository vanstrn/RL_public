...
class A3C(Method):
        ...
        if globalAC is None:   # get global network
            ...
            with tf.variable_scope(scope+"_update"):
                ...
                with tf.name_scope('c_loss'):
                    td_error = tf.subtract(self.td_target, self.psi, name='TD_error')
                    sf_error = tf.square(tf.add(td_error,self.phi, name="sf_error"))
                    self.c_loss = tf.reduce_sum(sf_error,name="sf_loss")
            ...
            with tf.name_scope('sync'):
                ...
                with tf.name_scope('push'):
                    ...
                    self.update_r_op = tf.train.AdamOptimizer(HPs["Actor LR"]).apply_gradients(zip(self.r_grads, globalAC.r_params))
            ...
            self.losses = [self.a_loss,self.c_loss,self.s_loss,self.r_grads]
    ...
    def Update(self,HPs,episode=0):
        ...
        for traj in range(len(self.buffer)):
            ...
            # a_loss, a_grads, c_loss, c_grads, s_loss=self.sess.run([self.a_loss, self.a_grads, self.c_loss, self.c_grads, self.s_loss], feedDict)   # local grads applied to global net.
