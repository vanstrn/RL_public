...
from methods.Agent57 import WorkerSlave,WorkerMaster,Agent57Buffer,SlidingWindowUCBBandit
...
with tf.device(args.processor):
    ...
    for i in range(settings["NumberENV"]):
        ...
        Method = GetFunction(settings["Method"])
        localNetwork = Method(network,sess,stateShape=dFeatures,actionSize=nActions,scope=i_name,HPs=settings["NetworkHPs"],globalNet=Updater,nTrajs=nTrajs,sharedBuffer=sharedBuffer,sharedBandit=sharedBandit)
