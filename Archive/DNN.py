...
def CreateDNN(inputShape,outputShape,optimizer=None,layerSizes=[100,100,100]):
    ...
    for i,size in enumerate(layerSizes):
        if i==0:
            model.add(Dense(size, input_dim=inputShape[0], activation="relu"))
        else:
...
def Create2InDNN(inputShape1,inputShape2,optimizer=None,layerSizes1=[100,100,100],
    ...
    for i,size in enumerate(layerSizes3):
        if i==0:
